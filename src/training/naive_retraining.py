...
def load_shard_dataloaders_task(
    ...
    train_shard_indices, train_indices, val_indices, test_indices = (
        dataset_splits_storage.retrieve_all_splits()
    )
...
def train_shards_task(
    ...
    for shard_idx, (
        ...
        trained_embedding_model = train_sharded_embedding_model(
            accelerator=accelerator,
            embedding_model=untrained_embedding_model,
            classifier=classifier,
            optimizer=shard_optimizer,
            train_dataloader=train_shard_dataloader,
...
def precompute_embeddings_task(
    ...
    for shard_idx, sispa_embedding_model in enumerate(
        ...
        training_datapoint_ids = [str(uuid.uuid4()) for _ in training_labels]
        validation_datapoint_ids = [str(uuid.uuid4()) for _ in validation_labels]
...
def train_aggregator_task(
    ...
    trained_aggregator = train_aggregation_classifier(
        ...
        optimizer=aggregator_optimizer,
...
def naive_retraining(
    ...
    embedding_model = ResNet(num_blocks=num_blocks, hidden_dim=hidden_dim)
    classifier = torch.nn.Linear(hidden_dim, num_classes)
    untrained_embedding_models = [embedding_model.clone() for _ in range(num_shards)]
    classifiers = [classifier.clone() for _ in range(num_shards)]
