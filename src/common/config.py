...
class ModelConfig(BaseModel):
    ...
    hidden_dim: int = Field(
        default=128, ge=1, description="Hidden dimension of the model"
    )
    num_blocks: int = Field(
        default=3, ge=1, description="Number of blocks in the model"
    )
...
class FinetuningConfig(BaseModel):
    ...
    limit_val_batches: int = Field(
        default=0.1, description="Number of validation batches to run", ge=1
    )
    precision: str = Field(
